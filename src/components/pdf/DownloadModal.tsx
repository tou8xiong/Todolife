...
export default function DownloadModal({ open, fileName, onFileNameChange, onConfirm, onClose }: Props) {
    if (!open) return null;
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-[90%] max-w-sm p-6 flex flex-col gap-4">
                <h2 className="text-lg font-bold text-gray-800 dark:text-white">💾 Save PDF As</h2>
                <div>
                    ...
                    <div className="flex items-center border-2 border-amber-300 rounded-xl overflow-hidden focus-within:border-amber-500 transition-colors">
                        ...
                        />
                        <span className="pr-3 text-sm text-gray-400 select-none">.pdf</span>
                    </div>
                </div>
